...
export const generateExaminers = (count: number): Examiner[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let j = 0; j < specializationCount; j++) {
      const area =
        technologyAreas[Math.floor(Math.random() * technologyAreas.length)]
...
export const generateWeeklyProductivity = (
  weeks: number
): WeeklyProductivity[] => {
...
export const generateApplications = (count: number): PatentApplication[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    applications.push({
      ...
      assignedExaminer:
        Math.random() > 0.2
          ? `Examiner ${Math.floor(Math.random() * 5) + 1}`
          : null,
...
export const getSummaryData = () => {
  const applicationsInQueue = mockApplications.filter(app =>
    [
      'New',
      'Verification',
      'Search',
  ...
  const myWorkspace = mockApplications.filter(
    app => app.assignedExaminer === 'Examiner 1'
  ).length
  const averageProcessingTime =
    mockApplications.reduce((sum, app) => sum + app.daysInCurrentStage, 0) /
    mockApplications.length
  const officeActionsThisMonth = mockApplications.reduce(
    (sum, app) => sum + app.officeActions,
    0
  )
...
export const getApplicationsByStage = () => {
  ...
  for (const app of mockApplications) {
    if (
      ['Search', 'Drafting', 'Examination', 'Publication'].includes(app.stage)
    ) {
