...
export default function LessonPage({ params }: { params: { unitId: string; lessonId: string } }) {
  ...
  // Find the unit and lesson data
  const unit = khoekhoegowabUnits.find((u) => u.id === params.unitId)
  const lesson = unit?.lessons.find((l) => l.id === params.lessonId)

  // If lesson not found, redirect to home
  useEffect(() => {
    if (!unit || !lesson) {
      router.push("/")
      return
    ...
    startLesson(params.unitId, params.lessonId)
  }, [unit, lesson, router, params.unitId, params.lessonId, startLesson])
  ...
  const stopListening = () => {
    if (recognitionRef.current) {
      ...
      // Simple check - in a real app, you'd use more sophisticated matching
      ...
      // Very simple check - in a real app, you'd use more sophisticated matching
      if (
        expectedPhrase &&
  ...
  const nextStep = () => {
    if (step < lessonSteps.length - 1) {
      ...
      if (currentPage < totalPages - 1) {
        ...
        completeLesson(params.unitId, params.lessonId, 100)
  ...
  return (
    <div className="container max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-sm text-muted-foreground">
          {currentPage * phrasesPerPage + step + 1} of {lesson.vocabulary.length + lesson.phrases.length} phrases
        </span>
      </div>
        ...
        <Card className="mb-6">
          <CardContent className="p-6 text-center">
            <div className="py-8 space-y-6">
              ...
              <h2 className="text-2xl font-bold">¡Excelente! Lesson Complete</h2>
              <p className="text-muted-foreground">
                You've successfully completed this lesson with over {lesson.vocabulary.length + lesson.phrases.length}{" "}
