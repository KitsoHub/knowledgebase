...
export default function CollectionDashboard({
  ...
  const handleDeleteCollection = () => {
    console.log(
      'Delete collection:',
      currentCollection?.collectionMetadataIdentifier
    )
  }
  ...
  const handleExportCollection = () => {
    console.log(
      'Export full data for community:',
      currentCollection?.collectionMetadataIdentifier
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-2">
        ...
        >
          ...
          <DialogContent className="max-w-6xl max-h-[90vh] overflow-auto">
            <DialogHeader>
              <DialogTitle>
                Contribute to {currentCollection?.title}
              </DialogTitle>
              <DialogDescription>
                Share knowledge in collection.
              </DialogDescription>
            </DialogHeader>
            <ItemContributionWizard
              onComplete={() => setShowContributeDialog(false)}
      ...
      <Alert className="border-secondary">
        ...
        <AlertDescription>
          ...
          {currentCollection?.curator?.name ||
            currentCommunity?.identity.leadership?.primaryContact.name}{' '}
          {currentCollection?.collectionMetadataIdentifier} and follows
          traditional protocols for {currentCollection?.description}.
        </AlertDescription>
      </Alert>
        ...
        <TabsContent value="overview" className="space-y-4">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {currentCollection?.rightsProtocols?.map(
                      (protocol, index) => (
                        <Badge
                          key={index}
                          variant="outline"
          ...
          <Card className="border-orange-200 bg-orange-50/50">
            <CardHeader>
              ...
              <CardDescription>
                Quick access to administrative functions and collection
                management
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h3>
                ...
                <span className="text-muted-foreground">
                  {currentCollection?.knowledgeItems?.length}
                </span>
              </h3>
              <Button onClick={() => setShowContributeDialog(true)}>
                <Plus className="w-4 h-4 mr-2" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(currentCollection?.knowledgeItems?.length ?? 0) > 0 ? (
                currentCollection?.knowledgeItems?.map(
                  collection => {
                    const isSelected = selectedItems.includes(
                      collection?.colletionId || ''
                    )
                    const IconComponent =
                      contentTypeIcons[collection.type as ContentType]
                    ...
                    return (
                      ...
                      >
                        <CardHeader className="space-y-3">
                          ...
                          <div
                            onClick={() =>
                              !selectMode && setSelectedItem(collection)
                            }
                          >
                        ...
                        >
                          ...
                          <div className="space-y-2 text-sm">
                            {collection.content?.duration && (
                              <div className="flex items-center space-x-2 text-muted-foreground">
                                <span>
                                  Duration: {collection.content.duration}
                                </span>
                              </div>
                            )}
                            {collection.content?.dimensions && (
                              <div className="flex items-center space-x-2 text-muted-foreground">
                                <span>
                                  Size: {collection.content.dimensions}
                                </span>
                              </div>
                            )}
                            <div className="flex items-center space-x-2 text-muted-foreground">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Badge
                              className={`text-xs ${accessLevelColors[collection?.rightsMetadata?.accessLevel.toString() as CulturalProtocol]}`}
                            >
                              {collection.rightsMetadata?.accessLevel.replace(
                                /_/g,
                                ' '
                              )}
                            </Badge>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {collection?.culturalMetadata?.tkLabels?.map(
                              label => (
                                <Badge
                                  key={label}
                                  variant="outline"
                                  className="text-xs tk-label tk-cultural"
                                >
                                  {label.replace(/_/g, ' ')}
                                </Badge>
                              )
