...
export default function FairUsageModal({isOpen, onClose}: FairUsageModalProps) {
 ...
 const handleClose = useCallback(() => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" id="fairUsageModal" aria-labelledby="fairUsageModalLabel" aria-hidden="true">
      <div className="relative w-full max-w-screen-md mx-4 bg-white rounded-2xl shadow-2xl border border-sky-100 max-h-[90vh] overflow-y-auto">
        <div className="relative p-10 pb-4">
          ...
          <div className="relative">
            <h2 className='text-2xl font-bold mb-4' id="fairUsageModalLabel">{fairUsagePolicy.title}</h2>
                      <Button
            variant="ghost"
            size="icon"
            onClick={handleClose}
          ...
          <div className="space-y-4">
            ...
            <li>{fairUsagePolicy.paragraphs[1].text}</li>
              ...
              <li>{fairUsagePolicy.paragraphs[3].text}</li>
               ...
               <p>{fairUsagePolicy.learnMore[0].text}<Link href={`${fairUsagePolicy.learnMore[0].link}`}
               className="ml-1 text-blue-600 underline">{fairUsagePolicy.learnMore[0].title}</Link></p>
          </div>
       <div className="flex flex-col gap-3 mt-6">
            <Button
             size='lg' variant='outline' className='text-base rounded-3xl'
            ...
            <Button type="button"
             ...
             onClick={handleAcknowledgement}> I acknowledge this, continue</Button>
          </div>
        </div>
      </div>
    </div>
